...
`;
        ...
        return (
            <>
                <section className="hero"><div className="hero-content"><h1>Swim for Life</h1><p className="tagline">Private Swim Lessons · Ages 3 &amp; Up</p><p className="hero-subtagline">Swimming · Diving · Stroke Work</p><button className="hero-button" onClick={() => handleBookingClick(null)}>Book a private lesson</button><button className="hero-button hero-button-secondary" onClick={() => { setShowAbout(a => !a); setTimeout(() => { if (!showAbout) document.getElementById('about').scrollIntoView({ behavior: 'smooth' }); }, 50); }}>About Coach Conor</button><button className="hero-button hero-button-secondary" onClick={() => { setShowContact(c => !c); setTimeout(() => { if (!showContact) document.getElementById('contact').scrollIntoView({ behavior: 'smooth' }); }, 50); }}>Contact Info</button></div></section>
