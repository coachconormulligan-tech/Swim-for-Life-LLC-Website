    const App = () => {
        ...
        const [preselectedPool, setPreselectedPool] = useState(null);
        ...
        const saveManualStudents = async (students) => {
            if (!db) return;
            try { await db.collection('swimLessons').doc('students').set({ data: students }); }
            catch (e) { console.error('Error saving students:', e); alert('Could not save student. Check Firestore rules — the "students" document may not be permitted.'); }
        };
        ...
        const syncStudentFromLesson = async (lessonInfo) => {
            ...
            const upsert = (name, birthday) => {
                if (!name) return;
                const key = name.toLowerCase().trim();
                const idx = students.findIndex(s => s.name && s.name.toLowerCase().trim() === key);
                const existing = idx >= 0 ? students[idx] : null;
                const record = {
                    id: existing?.id || ('st-' + Date.now() + '-' + Math.random().toString(36).slice(2, 7)),
                    name: name,
                    birthday: birthday || existing?.birthday || '',
                    parentName: lessonInfo.parentName || existing?.parentName || '',
                    email: lessonInfo.email || existing?.email || '',
                    phone: lessonInfo.phone || existing?.phone || ''
                };
                if (existing) {
                    if (existing.name !== record.name || existing.birthday !== record.birthday ||
                        existing.parentName !== record.parentName || existing.email !== record.email ||
            ...
            upsert(lessonInfo.swimmer1Name, lessonInfo.swimmer1Birthday);
            if (lessonInfo.lessonType === 'group' && lessonInfo.swimmer2Name) {
                upsert(lessonInfo.swimmer2Name, lessonInfo.swimmer2Birthday);
            }

            if (changed) {
                setManualStudents(students);
                await saveManualStudents(students);
            }
        };
        ...
        const handleLocationBookingClick = (poolId) => {
            setPreselectedPool(poolId);
            setPreselectedType('private');
            setAutoBookWeekly(false);
            setLessonTypeUpdate(prev => prev + 1);
            setShowCalendar(true);
            setTimeout(() => document.getElementById('calendar')?.scrollIntoView({ behavior: 'smooth' }), 100);
        };
