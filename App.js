...
`;
        ...
        return (
            <>
                ...
                <section className={`locations-section ${showLocations ? 'visible' : ''}`} id="locations"><div className="locations-content"><h2>Available Locations</h2>{pools.length === 0 ? <p className="locations-empty">No locations are currently listed. Check back soon!</p> : pools.map(pool => (<a key={pool.id} className="location-card" href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent((pool.name ? pool.name + ' ' : '') + (pool.address || ''))}`} target="_blank" rel="noopener noreferrer"><div className="location-name">{pool.name}</div>{pool.address && <div className="location-address">{pool.address}</div>}</a>))}</div></section>
