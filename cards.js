...
/* Layouts where the user picks one half to play (split cards, aftermath) */
const CHOOSEABLE_FACE_LAYOUTS = new Set(['split', 'aftermath']);
...
/* Resolve card data for the active face of a multi-face card.
   ...
   For split/room/adventure: combines both faces' oracle text. */
function _resolveCardFace(card, faceIndex) {
  ...
  // Room/Adventure: combine oracle text from all faces
...
function createPermanent(card, timestamp, opts = {}) {
  ...
  const perm = {
    ...
    manaValue:         resolvedCard.cmc || card.cmc || 0,
  ...
  if (card.card_faces && card.card_faces.length >= 2) {
    ...
    // For non-transformable, non-chooseable, non-room (adventure), store that all faces are active
...
function singularizeCreatureType(word) {
  ...
  // Already singular or same-form plural (merfolk, etc.)  ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â  capitalize
  const cap = low.charAt(0).toUpperCase() + low.slice(1);
  // Remove trailing 's' for standard plurals
...
function buildAuraRestriction(enchantTarget) {
  ...
  if (/\bplayer\b/.test(t) && !/\b(creature|artifact|planeswalker|enchantment|land|permanent)\b/.test(t)) {
    return () => false; // player-only auras can't target battlefield permanents
  }
...
function parseCardEffects(permanent, card, opts = {}) {
  ...
  if (/loses all creature types/i.test(oracle) && !_isLosesInActivatedAbility && !effects.some(e => e.params.losesAllCreatureTypes)) {
    // Determine scope: self or targeted
    ...
    effects.push({
      ...
      params: { losesAllCreatureTypes: true, replaceWith: null },
      appliesTo: null,
      scope: (isSelfLose || isEnchanted) ? 'targeted' : 'global',
  ...
  while ((setTypeMatch = setTypeRegex.exec(oracle)) !== null) {
    ...
    if (fLower.includes('if ') || fLower.includes('when ') || fLower.includes('whenever ') ||
        ...
        fLower.includes('life') || fLower.includes('spell') || fLower.length > 50) continue;
