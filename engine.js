  ...
  if (effect.appliesTo) {
    ...
    if (!groupAlreadyApplied) {
      ...
      if (effect.isSpellEffect && permanent) {
        ...
        const baseMatches = effect.appliesTo(baseState, allStates, effectCtrl);
        const currentMatches = effect.appliesTo(permState, allStates, effectCtrl);
        ...
        if (!effect.appliesTo(permState, allStates, effectCtrl)) return false;
      }
    }
  }
...
function evaluatePermanent(permanent, allPermanents, allEffects, inspectedId) {
  ...
  for (const layerDef of LAYERS) {
    ...
    // Fix 5: At the start of Layer 4, evaluate changeling for all permanents.
    // isAllCreatureTypes is set here based on whether changeling exists at this point.
    // If changeling was lost in Layers 1-3, the creature will NOT get all creature types.
    // If changeling was gained in Layers 1-3, the creature WILL get all creature types.
    // Once set in Layer 4, isAllCreatureTypes persists even if changeling is removed in Layer 6.
    if (layerDef.id === '4') {
      for (const [pid, st] of allStates) {
        const currentHasChangeling = st.abilities.some(a => /\bchangeling\b/i.test(a));
        st.isAllCreatureTypes = currentHasChangeling;
        st.hasChangeling = currentHasChangeling;
      }
    }
