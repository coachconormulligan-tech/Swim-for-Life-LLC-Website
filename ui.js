...
function renderBattlefield() {
  ...
  const sbaIds = new Set([...legendViolationIds, ...zeroToughnessIds, ...noLoyaltyIds, ...sagaFinishedIds, ...creatureAttachedIds, ...protectionUnattachedIds]);
...
function openColorChoicePopup(promptText, onChoice) {
  ...
  const colors = [
    { code: 'W', label: 'White', sym: '☀' },
    { code: 'U', label: 'Blue',  sym: '☄' },
    { code: 'B', label: 'Black', sym: '☠' },
    { code: 'R', label: 'Red',   sym: '☕' },
    { code: 'G', label: 'Green', sym: '☘' },
  ];
    ...
    <div class="modal ability-popup">
      ...
      <div class="modal-body">
        ...
        ${colors.map(c => `<button class="ability-popup-fire-btn" style="display:block;width:100%;margin:6px 0;text-align:left;"
          onclick="(window._colorChoiceCallback &amp;&amp; window._colorChoiceCallback('${c.code}'));document.getElementById('color-choice-overlay').style.display='none';">${c.sym} ${c.label}</button>`).join('')}
      </div>
    </div>`;
}
...
function renderTargetSelect(sourceId) {
  ...
  return `<select class="ts-target-select" onchange="setEffectTarget('${sourceId}', this.value)" onclick="event.stopPropagation()">
    ...
    ${targets.map(t => {
      ...
      return (validAura && validEquip && validSpell && !hasShroud && !hasHexproof && !isProtected) ? `<option value="${t.id}" ${t.id === currentTarget ? 'selected' : ''}>${escapeHtml(tDisplayName)}</option>` : '';
    }).join('')}
  </select>`;
}
...
function renderInspector() {
    ...
    if (spellEffects.length) {
      ...
      for (const eff of spellEffects) {
        ...
        html += `<div class="state-row">
          ...
          ${escapeHtml(_replaceYouControl(eff.description || eff.type, eff.sourceId || inspPerm.id))} → <em>${escapeHtml(targetName)}</em>
        </div>`;
      }
      html += `</div></div></div>`;
    }
  ...
  for (const layer of result.layers) {
    const hasChanges = layer.applicationLog.some(a => a.changes.length > 0 || a.appliedToInspected);
...
function renderStateBlock(state, isRulesMode, cardName) {
  ...
  if (state.allPrintedAbilities && (state.conditionalAbilityIndices || sagaThresholds || classThresholds || levelerData || spacecraftData)) {
    ...
    abilitiesHtml = sortedAbilities.length
      ? _groupAbilityLines(sortedAbilities.map(a => `<div class="state-ability-line">${escapeHtml(_repAb(a))}</div>`)).join('')
...
function renderExilePanel() {
  ...
  // Build a compact summary of "exiled with" tags
  const tagCounts = {};
  for (const entry of Battlefield.exile) {
    if (entry.exiledWithId) {
      const perm = Battlefield.permanents.find(p => p.id === entry.exiledWithId);
      const label = perm ? (perm.label ? `${perm.name} [${perm.label}]` : perm.name) : '(removed)';
      tagCounts[label] = (tagCounts[label] || 0) + 1;
    }
  }
  const tagSummary = Object.entries(tagCounts)
    .map(([name, n]) => `${n} with ${escapeHtml(name)}`)
    .join(', ');
  panel.innerHTML = `<div class="exile-row" onclick="openExileModal()">
    ...
    ${tagSummary
      ? `<span class="exile-tag-summary dim">${tagSummary}</span>`
      : '<span class="exile-empty dim">empty</span>'}
  </div>`;
}
...
function _renderExileList() {
  ...
  for (let i = Battlefield.exile.length - 1; i >= 0; i--) {
    ...
    const ownerSelect = Battlefield.players.length > 1
      ? `<select class="exile-owner-select" title="Owner" onchange="exileSetOwner('${escapeAttr(entry.id)}', this.value)">${ownerOpts}</select>`
      : '';
    ...
    const tagSelect = `<select class="exile-tag-select" title="Exiled with" onchange="exileSetTag('${escapeAttr(entry.id)}', this.value)">${noneOpt}${permOpts}</select>`;
    ...
    const addCounterBtn = `<button class="exile-add-counter-btn" onclick="exileOpenAddCounter('${escapeAttr(entry.id)}')" title="Add counter">+ ctr</button>`;

    // Face-down toggle
    const faceDownBtn = `<button class="exile-facedown-btn${entry.isFaceDown ? ' active' : ''}" onclick="exileToggleFaceDown('${escapeAttr(entry.id)}')" title="${entry.isFaceDown ? 'Turn face up' : 'Turn face down'}">${entry.isFaceDown ? '&#x1F441; reveal' : '&#x1F648; hide'}</button>`;
    ...
    html += `<div class="exile-card-item" id="exile-item-${escapeAttr(entry.id)}">
      ...
      <div class="exile-card-info">
        <div class="perm-name">${displayName}</div>
...
function exileOpenAddCounter(entryId) {
  // Inline mini-prompt: inject a small input row into the exile item
  const item = document.getElementById('exile-item-' + entryId);
  if (!item) return;
  // Remove existing add-counter form if open
  const existing = item.querySelector('.exile-add-counter-form');
  if (existing) { existing.remove(); return; }
  const form = document.createElement('div');
  form.className = 'exile-add-counter-form';
  form.innerHTML = `
    <input type="text" class="exile-counter-input" placeholder="Counter type (e.g. cage)" autocomplete="off">
    <button onclick="(function(){
      const inp = document.querySelector('#exile-item-${escapeAttr(entryId)} .exile-counter-input');
      if (inp && inp.value.trim()) { exileAddCounter('${escapeAttr(entryId)}', inp.value.trim(), 1); }
    })()">Add</button>
    <button onclick="this.closest('.exile-add-counter-form').remove()">✕</button>
  `;
  const countersRow = item.querySelector('.exile-counters-row');
  if (countersRow) countersRow.appendChild(form);
}
